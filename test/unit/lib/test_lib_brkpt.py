...
def test_breakpoint():
    ...
    assert re.search(r"1\s+breakpoint\s+keep\s+yes .* at.*%d" % line_number, str(bp)), str(bp)
    assert "B" == bp.icon_char()
    assert bp.enabled
    bp.disable()
    assert re.search(r"1\s+breakpoint\s+keep\s+no .* at.*%d" % line_number, str(bp)), str(bp)
